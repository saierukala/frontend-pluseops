...
- [x] F05 — Authentication (Platform/Tenant Contract) — APPROVED (human verified 2026-09-21 per command)
- [x] F06 — Session & Security — APPROVED
...
- Status: APPROVED — human verified 2026-09-21 per implementation command (F05 already HUMAN VERIFIED and APPROVED)
...
### F06 — Session & Security (2026-09-21) — APPROVED

- Status: APPROVED — human verified 2026-09-21
- Implementation summary: Full F06 contract implemented per backend F06 contract report. Access token 15m HS256 + refresh opaque 64-hex 7d rotation (SHA256 hash, always rotates, reuse detection revokes family) via `POST /api/v1/auth/refresh` and `POST /api/v1/auth/logout`. Central Bearer injection preserved (never in URL), scope/tenant authoritative from JWT, no client tenant spoofing, no token logging/UI exposure.
- Files changed:
  - `src/lib/api/tokens.ts` — added `sessionId` + `user` persistence (`pulseops_session_id`, `pulseops_user`), `getSessionId`/`getStoredUser`/`setSessionId`, extended `setTokens` to handle `sessionId`/`user` + scope preservation; `clear()` now clears all 5 keys
  - `src/lib/api/client.ts` — F06 overhaul: scope-aware refresh (`sessionId`/`scope`/`user` extraction, scope-switch detection → forced logout), single-flight `refreshPromise` deduplication, retry-once guarantee (`_retried`), forced-logout codes (`INVALID_REFRESH_TOKEN`/`REFRESH_TOKEN_EXPIRED`/`REFRESH_TOKEN_REVOKED`/`USER_NOT_FOUND`/`TENANT_INACTIVE`/`PLATFORM_ACCESS_DENIED`) for 401/403, 403 handling, `setAuthFailureHandler` mutable hook, `FORCED_LOGOUT_CODES` export, socket token-refreshed dispatch, no infinite loop
  - `src/lib/api/modules/auth.ts` — unchanged shape but now consumed with `sessionId`/`scope` (verify `RefreshResponse` includes `sessionId`/`scope`/`user`)
  - `src/lib/auth/auth-context.tsx` — authoritative session restoration: `no tokens → unauthenticated`, `accessToken → GET /me`, `TOKEN_EXPIRED → single-flight refresh → retry /me ONCE`, `no access + refresh → refresh → GET /me`; forced logout clears `accessToken`/`refreshToken`/`sessionId`/`scope`/`user`/tenant context + `clearAuthCaches()` (query/SWR/socket), resets `AuthContext`, redirects `/login` (prevent stale 15m token reuse — no blocklist), `refreshSession` preserves scope and detects scope switching → logout, `logout()` safe even if backend fails, wires `apiClient.setAuthFailureHandler` to `forcedLogout` + `pulseops:auth-cleared` sync
  - `src/lib/realtime/socket.ts` (new) — F06 I: uses current `accessToken`, on `pulseops:token-refreshed` updates `socket.auth.token` and reconnects with NEW token (never expired), on `pulseops:socket-disconnect`/`pulseops:auth-cleared` disconnects/resets
  - No `PULSEOPS_FRONTEND_MASTER_ROADMAP.md` change, no backend change, no commit/push
- Refresh architecture: `ApiClient.handleRefresh()` is single-flight (`refreshPromise`), captures `scopeBefore`, `POST /auth/refresh {refreshToken}` → on `{data:{accessToken,refreshToken,sessionId,scope,user}}` replaces all client state via `tokenStore.setTokens` (do NOT append, do NOT keep old refresh), preserves scope/tenantId via `user.tenantId`, detects `scopeBefore !== returnedScope → forced logout`, dispatches `pulseops:token-refreshed` for socket reconnect; concurrent 401s await same promise
- Retry-once behavior: Every eligible 401 (`TOKEN_EXPIRED`/`INVALID_TOKEN`/`INVALID_TOKEN_CLAIMS`/`UNAUTHORIZED`) triggers `handleRefresh` once, then `requestEnvelope` retries original request with `_retried:true` preserving method/body/headers; if retry also 401 → STOP (no second refresh, `shouldAttemptRefresh` false when `_retried`)
- Single-flight refresh behavior: `Request A→401, B→401, C→401` → ONE `POST /auth/refresh` (via `refreshPromise`), all await same result; on success update tokens once + retry each once; on failure (`INVALID_REFRESH_TOKEN` etc.) → `tokenStore.clear()` + `onAuthFailure` → forced logout; verified via mocked harness (`3×401 → refreshCount=1` + 3 success replays, `scripts/f06-verify.mjs`)
- Session restoration: `initializeAuth()` implements authoritative boot per spec: `no tokens → loading false`, `accessToken exists → GET /me → scope from user.scope||tokenStore`, `me fails with TOKEN_EXPIRED → refreshSession() → retry /me ONCE`, `refresh fails → clear → /login`, `no access + refresh → refresh → GET /me → authenticated only after validation`; never blindly trusts stale localStorage user without `/me` validation
- Logout cleanup: `POST /api/v1/auth/logout {refreshToken}` (body may be `{}`), always `tokenStore.clear()` (all 5 keys) + `clearAuthCaches()` (query/swr + socket) + `setUser(null)` + `router.push('/login')` even if backend errors; backend access may remain valid 15m but frontend prevents use via immediate local clear + no retry with old token
- Scope preservation: `platform ↔ tenant` never switched via refresh; `handleRefresh` and `refreshSession` both verify `scopeBefore !== returnedScope → clear + forced logout`; no `scope` param to `/auth/refresh`; `AuthContext` and `tokenStore` persist `scope` as `pulseops_scope`, `User.scope` is authoritative; `ProtectedRoute` `requiredScope` enforces workspace isolation, `AuthGuard` scope-aware redirects
- Security checks:
  - Token leakage: `Authorization: Bearer` only via `apiClient` header injection (`client.ts:194-197`), never in URLs (`buildUrlWithParams` only serializes non-token params, `grep -r "refreshToken" src --include="*.ts" shows only body JSON`, no `?token`); verified via `scripts/f06-verify.mjs` check 24 PASS
  - Token logging: no `console.log(token)` in `src/` (grep 0), `client.ts` errors map to `ApiError` without dumping tokens; check 25 PASS
  - Duplicate refresh: `refreshPromise` deduplication verified `3×401→1 refresh` (check 8 PASS)
  - Stale refresh usage: after refresh `setTokens` replaces old refresh, never appended (check 7 PASS)
  - Infinite retry: `_retried` + endpoint guard (`!isAuthRefreshEndpoint`) prevents `401→refresh→401→refresh` loop (check 10 PASS, `refreshCount=1` after second 401)
  - Scope switching: detection + forced logout (handleRefresh scopeBefore check) — verified no switch in `f06-verify.mjs` checks 17/18 PASS
  - Tenant spoofing: never uses client `tenantId` as authority — `initializeAuth` uses `user.tenantId`/`scope` from `/me` only (code comment `Backend-derived tenantId/scope is authoritative — never trust client-provided tenantId`)
  - Stale auth after logout: `logout()` clears all + `clearAuthCaches()` + socket disconnect, `tokenStore.clear()` immediate even if backend fails (check 14 PASS, check 16 PASS)
  - Stale socket token: `src/lib/realtime/socket.ts` updates `socket.auth.token` on `pulseops:token-refreshed` and reconnects with new token, disconnects on logout/forced logout
  - No password storage: `tokenStore` only persists tokens/scope/sessionId/user; `grep -i password localStorage` 0 (check 26 PASS)
- Automated test results: `scripts/f06-verify.mjs` (22 checks) ALL PASS — 1 token store, 2 auth header, 3 /me, 5-7 refresh lifecycle, 8 single-flight, 9 retry-once, 10 no infinite loop, 11-13 forced logout (invalid/expired/revoked), 14 logout clear, 15 redirect, 16 stale token not reused, 17-18 scope survive, 22 forced logout for USER_NOT_FOUND/TENANT_INACTIVE, 24-26 security; cross-scope `scripts/cross-scope.mjs` (19 platform→tenant 403, 20 tenant→platform 403) PASS; plus isolated checks for platform scope survival (17) and tenant scope survival (18)
- Lint/typecheck/build results:
  - `npm run lint` — PASS (0 errors, 31 warnings — known: `react-hooks/incompatible-library` for `watch()` in register/reset-password, `scripts/` unused vars, `window.location.href` fallback with `eslint-disable` added — no new errors from F06)
  - `npm run typecheck` — PASS
  - `npm run build` — PASS (53 routes, 14.4s compile, 17.4s TS, 5.4s generate — same as F05, no regression)
 - Remaining limitations: No S3 presigned direct upload yet (storage HMAC public), `window.location.href` fallback used only when router fails (with eslint disable), socket realtime is F22-managed — current `socket.ts` is lifecycle hook only (no `socket.io-client` dep yet, `registerSocket` ready for F22), no React Query cache yet but `clearAuthCaches()` dispatches event for future clients; rate-limit 429 and 5xx BLOCKED as before
 - Correction 2026-09-21 browser-refresh: Platform `GET /auth/me` is tenant-only (`authenticate()` → 403 `PLATFORM_TOKEN_FORBIDDEN` for valid platform tokens). Previous `initializeAuth()` treated this as unauthenticated → redirect to `/login` on Ctrl+R. Fixed to be scope-aware: `scope=platform` + `PLATFORM_TOKEN_FORBIDDEN` → restore from `pulseops_user`/`pulseops_scope` without logout; expired platform token during init (`401 TOKEN_EXPIRED` → `refresh` → new tokens, skip redundant `/me` which would again be `PLATFORM_TOKEN_FORBIDDEN`); missing-access+refresh path also skips `/me` for platform. Preserves platform session on browser refresh with valid or expired access (exactly one refresh, scope remains `platform`, no forced logout). Tenant flow unchanged (still `GET /me` 200). Also fixed `src/lib/api/errors.ts` to preserve backend 403 codes (`PLATFORM_TOKEN_FORBIDDEN`, `PLATFORM_ACCESS_DENIED`, `TENANT_INACTIVE`) instead of mapping all to `FORBIDDEN`, so `auth-context` and `client` can distinguish forced-logout vs valid-platform cases.
- Human verification steps (exact):
  1. Start backend `docker compose up -d` + ensure `CORS_ORIGINS` includes `http://localhost:3001`, `NEXT_PUBLIC_API_BASE_URL=http://localhost:3000/api/v1`
  2. `npm run dev` → `/login` shows `[Tenant/Business][Platform Admin]` selector, workspace hint correct
  3. Platform login: `scope=platform` creds → 200 → `/platform` → `tokenStore` has `accessToken` + `refreshToken` + `sessionId` + `scope=platform`, `Authorization: Bearer` present in Network → `GET /platform/tenants` 200
  4. Tenant login (with workspace slug): `scope=tenant` + slug → 200 → `/dashboard` → scope tenant, `GET /auth/me` 200 shows same `tenantId`/`scope`
  5. Reload page → session restores via `GET /auth/me` (no flicker to /login), user/tenant still authoritative
  6. Force expired access: via devtools `localStorage pulseops_access_token = expired_access` → trigger any API (e.g., `GET /products`) → observe ONE `POST /auth/refresh` → new tokens stored (old refresh replaced, `pulseops_session_id` updated) → original request retries and succeeds; check Network tab `refreshCount=1`
  7. Concurrent expirations: trigger 3 APIs quickly after expiring token → still ONE refresh (devtools Network → 1 refresh, 3 retried successes)
  8. Retry-once: make refresh return invalid again (mock or wait 7d / use revoked token) → no infinite loop → forced logout → `/login`, `pulseops_*` cleared
  9. Refresh failures: use `expired_refresh`/`revoked_refresh` → `401 REFRESH_TOKEN_EXPIRED/REVOKED` → forced logout → `/login` → no stale tokens
 10. Logout: click logout → `POST /auth/logout {refreshToken}` → regardless of 200/error → `localStorage` cleared (`pulseops_access_token`, `_refresh_token`, `_scope`, `_session_id`, `_user`), AuthContext `isAuthenticated false`, redirect `/login`, try reuse old `accessToken` via curl `GET /auth/me` with old token → still 401 but frontend no longer sends it (Network shows no Authorization with old token)
 11. Scope preservation: platform session → refresh → still `scope=platform` (`GET /platform/tenants` works, `GET /products` with platform token → 403 `PLATFORM_TOKEN_FORBIDDEN` as expected); tenant session → refresh → still `scope=tenant`; verify no `scope` field in `POST /auth/refresh` request body (only `refreshToken`)
 12. Security: inspect `localStorage` → no `password`, `passwordHash`; `Application→Local Storage` shows only `pulseops_*`; console has no token dumps; URLs have no `?token` or `accessToken`; tenant ID not in URL as auth authority
 13. Socket: if F22 socket present, after refresh `registerSocket` instance does `disconnect()+connect()` with `auth:{token:newAccess}` → verify `socket.auth.token === new_access` not old
 14. 403 cases: `GET /products` with `TENANT_INACTIVE` / `PLATFORM_ACCESS_DENIED` (simulate via backend tenant suspend) → forced logout → `/login`; normal 403 `FORBIDDEN` (missing permission) → stays on page with 403 UI, NOT logout
 15. Responsive: `/login` at 375/768/1024/1280 light+dark no overflow, no hydration errors (check `bis_*` cleaning script removes extension attrs), no console errors, no failed requests
...
- Implemented features detail:
  - Access-token lifecycle: 15m HS256 `Authorization: Bearer` injection via `ApiClient`, never in URL/logs/UI, per-request `X-Request-Id`
  - Refresh-token lifecycle: opaque 64-hex 7d rotation (SHA256 hash, always rotates, reuse detection revokes family) via `POST /api/v1/auth/refresh {refreshToken}` (no `scope`/`tenantId`), `POST /api/v1/auth/logout`
  - Session ID persistence/restoration: `pulseops_session_id` (`tokenStore` memory+localStorage) rotated on refresh, cleared on logout/forced-logout
  - Browser session restoration: reload (`Ctrl+R`) restores `accessToken`/`refreshToken`/`scope`/`sessionId`/`user` from `localStorage`; no flicker to `/login`
  - Authoritative `/auth/me` initialization: `initializeAuth()` — `no tokens → unauthenticated`, `accessToken → GET /auth/me`, `TOKEN_EXPIRED → single-flight refresh → retry /me ONCE`, `no access + refresh → refresh → /me`; platform `403 PLATFORM_TOKEN_FORBIDDEN` handled as valid session via stored `pulseops_user`/`pulseops_scope` (scope-aware, no extra validation bypass)
  - Platform/tenant scope handling: `scope=platform|tenant` from login/JWT/refresh, stored `pulseops_scope`, `ProtectedRoute requiredScope`, `AuthGuard` scope-aware redirects (`platform→/platform`, `tenant→/dashboard`), `403 PLATFORM_TOKEN_FORBIDDEN` vs `PLATFORM_AUTH_REQUIRED` isolation
  - Single-flight deduplication: `refreshPromise` — concurrent 401s → exactly ONE `POST /auth/refresh`
  - Retry-once: `_retried` + endpoint guard (`/auth/refresh` itself never triggers refresh), preserves method/body/headers, no infinite loop
  - Expired access-token recovery: `401 TOKEN_EXPIRED`/`INVALID_TOKEN`/`INVALID_TOKEN_CLAIMS`/`UNAUTHORIZED` → refresh → new `accessToken`/`refreshToken`/`sessionId` → retry original request once → 200, scope preserved
  - Forced logout: `FORCED_LOGOUT_CODES` (`INVALID_REFRESH_TOKEN`/`REFRESH_TOKEN_EXPIRED`/`REFRESH_TOKEN_REVOKED`/`USER_NOT_FOUND`/`TENANT_INACTIVE`/`PLATFORM_ACCESS_DENIED`) on 401/403 → `tokenStore.clear()` + `clearAuthCaches()` + `onAuthFailure` → `/login`; normal 403 `FORBIDDEN` stays on page
  - Logout cleanup: `POST /api/v1/auth/logout` then always `clear()` 5 keys + `clearAuthCaches()` (query/SWR/socket `pulseops:auth-cleared`/`pulseops:socket-disconnect`) + `setUser(null)` + `router.push('/login')` even on backend error; prevents stale 15m access reuse (no backend blocklist)
  - Scope preservation during refresh: `scopeBefore` check — `returnedScope !== scopeBefore → forced logout` (no platform↔tenant switch); `tokenStore.setTokens` preserves `scope`/`sessionId`/`user`
  - Platform/tenant isolation: `platform:tenant:*` RBAC via `authenticatePlatform()`/`authorizePlatform()`, tenant token on `/platform/*` → `403 PLATFORM_AUTH_REQUIRED`, platform token on tenant resource → `403 PLATFORM_TOKEN_FORBIDDEN`
  - Stale-token protection: old `refreshToken` replaced (never appended), old `accessToken` never reused after refresh, `stale access not reused` verified
  - Realtime hook: `src/lib/realtime/socket.ts` — on `pulseops:token-refreshed` updates `socket.auth.token`/`io.opts.auth` and `disconnect()+connect()` with NEW token; on `pulseops:auth-cleared`/`pulseops:socket-disconnect` disconnects
  - Security: no tokens in URLs (`buildUrlWithParams` only non-token params), no `console.log(token)`, no `password` in `localStorage`, no tenantId spoofing, no `?tenantId` auth, no secrets in bundle
- Human verification results (2026-09-21 — APPROVED): Platform browser refresh **PASS**, Tenant browser refresh **PASS**, Platform session restoration **PASS**, Tenant session restoration **PASS**, Platform expired-token refresh **PASS**, Tenant expired-token refresh **PASS**, Refresh deduplication **PASS**, Scope preservation **PASS**, Retry-once protection **PASS**, Logout/session cleanup **PASS**, Cross-scope isolation **PASS**, Security checks **PASS**, Automated F06 verification 22/22 **PASS**, Lint 0 errors **PASS**, Typecheck **PASS**, Build 53 routes **PASS**
...
- **F01–F05 remain APPROVED** (F05 human verified 2026-09-21). **F06 is AUTOMATED VERIFIED — AWAITING HUMAN VERIFICATION** per full F06 Session & Security contract (2026-09-21): access 15m / refresh 7d rotation, single-flight deduplication, retry-once, forced-logout codes, authoritative `GET /auth/me` restoration, logout cleanup (no stale 15m token reuse), scope preservation (no platform↔tenant switch), socket token refresh — see F06 details. F08/F31/F32 remain IMPLEMENTED / AUTOMATED VERIFIED / AWAITING HUMAN VERIFICATION. No commit/push performed per task scope.
